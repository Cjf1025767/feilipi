Ext.define('Tab.view.extensetting.recordMonitorModel', {
    ...
    fields: [{
        ...
        },
        {
            ...
            convert: function(value, record) {
                ...
                if (ready) {
                    return '<div class="phone-item"><i class="fa fa-user fa-3x phone-item-agent"></i>&nbsp;<i class="fa fa-headphones phone-item-agent">&nbsp;'+username + '</i></div>';
                } else if (acw) {
                    return '<div class="phone-item"><i class="fa fa-user-times fa-3x phone-item-agent" style="color:#e6c558"></i>&nbsp;<i class="fa fa-headphones phone-item-agent">&nbsp;'+username+ '</i></div>';
                }
                return '<div class="phone-item"><i class="fa fa-user fa-3x phone-item-agent" style="color:#e68c58"></i>&nbsp;<i class="fa fa-headphones phone-item-agent">&nbsp;'+username+ '</i></div>';
            }
        }
    ]
});

Ext.define('Tab.view.extensetting.recordMonitorView', {
    ...
    requires: [
        ...
        'Ext.chart.*',
        'Tab.view.extensetting.recordMonitorModel'
    ],
    onEvent: function(message) {
        ...
        try {
            ...
            if (objs.sender == 'Monitor') {
                ...
                if (objs.init) {
                    console.log("输出监控init事件消息"+JSON.stringify(objs))
                    if (objs.extensions && typeof objs.extensions != 'undefined') {
                        ...
                        self.chart.up().down('button[name=all]').setText('All(' + objs.extensions.length + ')');
                        var nready = 0,
                            nnotready = 0,
                            nbusy = 0,
                            nacw = 0,
                        ...
                        self.chart.up().down('button[name=ready]').setText('Free(' + nready + ')');
                        self.chart.up().down('button[name=notready]').setText('Rest(' + nnotready + ')');
                        self.chart.up().down('button[name=nbusy]').setText('Busy(' + nbusy + ')');
                        self.chart.up().down('button[name=acw]').setText('Reprocessing(' + nacw + ')');
                        self.chart.up().down('button[name=logout]').setText('Logout(' + nlogout + ')');
                    }
        ...
        afterrender: function() {
            ...
            window.TabMonitor = new function() {
                ...
                this.wsReconnect = function() {
                    ...
                    try {
                        this.ws = new window.WebSocket(this.wshost + '/?Library=GetEvents&&userguid='+self.userguid, 'chat');
                    } catch (e) {}
                        ...
                        },
                        {
                          ...
                          },
                          {
                            ...
                            toggleHandler: function(btn, state) {
                                ...
                                var userguid=this.up('monitorView').userguid
                                var queues= container.down('tagfield[fieldLabel="MoreQueues"]').getValue()
                                var queuesstr=queues.toString();
                                Ext.Ajax.request({
                                    url: '/tab/rbac/GetMonitorQueues',
                                    method: 'POST',
                                    headers: {
                                        'Content-Type': 'application/x-www-form-urlencoded'
                                    },
                                    params: {
                                        department: container.down('combobox[name="department"]').getValue(),
                                        queues: queuesstr,
                                        userguid:userguid
                                    },
                                    async: false,
    ...
    items: {
        ...
        store: Ext.create('Ext.data.Store', {
            model: 'Tab.view.extensetting.recordMonitorModel',
            sorters: [{
                property: 'extension',
                direction: 'ASC'
            }]
